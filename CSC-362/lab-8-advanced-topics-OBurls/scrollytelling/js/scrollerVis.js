class ScrollerVis {
  ...
  step1() {
    ...
    vis.rect.transition()
      .attr('fill', d => d.difficulty === 'Easy' ? 'orange' : vis.colorScale('inactive'));
}
  ...
  step2() {
    ...
    // Change the colour of other rectangles
      vis.rect.transition()
      .attr('fill', d => d.difficulty === 'Difficult' ? 'purple' : vis.colorScale('inactive'));
  }
  ...
  step3() {
    ...
    vis.rect.transition()
        .attr('opacity', 1)
        .attr('fill', d => d.name==vis.dataLongestHike.trail ? vis.colorScale('highlighted') : vis.colorScale('inactive'))
        .attr('width', d => vis.config.cellWidth)
        .attr('height', d => vis.config.cellHeight)
        .attr('x', (d, i) => i % vis.config.columns * (vis.config.cellWidth + vis.config.cellSpacing))
        .attr('y', (d, i) => Math.floor(i / vis.config.columns) % vis.config.rows * (vis.config.cellHeight + vis.config.cellSpacing));
